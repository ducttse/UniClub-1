...
{
    public class CreateSchoolAdminCommandHandler : IRequestHandler<CreateSchoolAdminDto, string>
    {
        ...
        public CreateSchoolAdminCommandHandler(IIdentityService identityService, UserManager<Person> userManager, IUploadService uploadService, IUniversityRepository universityRepository, IMapper mapper)
        {
        ...
        public async Task<string> Handle(CreateSchoolAdminDto request, CancellationToken cancellationToken)
        {
            ...
            var university = await _universityRepository.GetByIdAsync(cancellationToken, new GetUniversityByIdQuerySpecification(new GetUniversityByIdDto(request.UniId)));
            if (university == null)
            {
                throw new NullReferenceException($"University {university.Id} not found");
            }
            var result = await _identityService.CreateUserAsync(_mapper.Map<Person>(request), request.Password);
            await _identityService.AddToRoleAsync(result.UserId, Role.SchoolAdmin.ToString());
            var user = await _userManager.FindByIdAsync(result.UserId);
            if (user == null)
            {
                throw new Exception("Error in creation user");
            }
            await _userManager.AddClaimAsync(user, new Claim("university", university.Id.ToString()));
